import {environment} from "../../../../environments/environment";
export const iconDescriptionShowSvg = '<svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 1C5.9 1 1 5.9 1 12C1 18.1 5.9 23 12 23C18.1 23 23 18.1 23 12C23 5.9 18.1 1 12 1ZM12 21C7 21 3 17 3 12C3 7 7 3 12 3C17 3 21 7 21 12C21 17 17 21 12 21ZM13.3 16.8C13.4 17 13.4 17.2 13.4 17.4C13.4 17.8 13.2 18.2 13 18.5C12.7 18.8 12.3 18.9 11.9 18.9C11.5 18.9 11.1 18.7 10.8 18.5C10.5 18.2 10.4 17.8 10.4 17.4C10.4 17 10.6 16.6 10.8 16.3C11.1 16 11.5 15.8 11.9 15.8C12.5 15.9 13 16.2 13.3 16.8ZM16 9.3C16 10.4 15.5 11.2 14.6 11.9C14.4 12.1 14.1 12.2 13.9 12.4L13.3 12.8C13 13 12.9 13.1 12.9 13.1C12.8 13.2 12.8 13.3 12.8 13.4C12.8 13.6 12.7 13.8 12.7 14.1V14.4V14.5C12.6 14.7 12.4 14.9 12.2 15C12 15.1 11.9 15.1 11.8 15.1H11.7C11.6 15.1 11.6 15.1 11.5 15.1C11.3 15 11.1 14.9 11 14.8C10.9 14.6 10.7 14.4 10.7 14.1C10.6 13.8 10.5 13.6 10.5 13.3C10.5 13 10.6 12.7 10.7 12.5C10.8 12.3 11 12.1 11.2 11.9C11.4 11.7 11.6 11.6 11.8 11.5L12.4 11.2C13 10.9 13.2 10.7 13.3 10.6C13.6 10.3 13.7 9.9 13.7 9.5C13.7 9.1 13.6 8.7 13.3 8.5C12.8 8 12.4 7.9 12 7.9C10.9 7.9 10.6 8.4 10.6 8.8C10.4 9.3 10.4 9.4 10.4 9.4V9.6C10.4 9.7 10.4 9.7 10.4 9.8L10.5 10.2L10.2 10.3C9.6 10.6 9.1 10.6 8.7 10.4C8.6 10.3 8.4 10.1 8.3 10C8.1 9.7 8 9.4 8 9C8 8.6 8.1 8.3 8.2 8C8.3 7.7 8.5 7.5 8.7 7.3C8.9 7.1 9.1 6.9 9.4 6.7C9.7 6.6 10 6.4 10.3 6.3C11.5 5.9 12.7 5.9 13.6 6.1C14.1 6.2 14.5 6.4 14.9 6.6C15.6 7.3 16 8.1 16 9.3Z" /></svg>';

export const iconDescriptionHideSvg = '<svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M2.3 3.7L3.6 5C2 6.9 1 9.3 1 12C1 18.1 5.9 23 12 23C14.7 23 17.1 22 19 20.4L20.3 21.7L21.7 20.3L3.7 2.3L2.3 3.7ZM12 21C7 21 3 17 3 12C3 9.9 3.7 7.9 5 6.4L10.8 12.2C10.7 12.3 10.6 12.4 10.6 12.5C10.5 12.7 10.4 13 10.4 13.3C10.4 13.6 10.5 13.9 10.6 14.1C10.7 14.3 10.8 14.5 10.9 14.8C11 15 11.2 15.1 11.4 15.1C11.5 15.1 11.5 15.1 11.6 15.1H11.7C11.8 15.1 11.9 15.1 12.1 15C12.4 14.9 12.6 14.7 12.6 14.5V14.4V14.1L17.5 19C16.1 20.3 14.1 21 12 21ZM7.8 4.1L6.3 2.6C8 1.6 9.9 1 12 1C18.1 1 23 5.9 23 12C23 14.1 22.4 16 21.4 17.7L19.9 16.2C20.6 15 21 13.5 21 12C21 7 17 3 12 3C10.5 3 9 3.4 7.8 4.1ZM13.5 9.8C13.5 9.7 13.6 9.6 13.6 9.4C13.6 9 13.5 8.6 13.2 8.4C12.8 8 12.4 7.9 12 7.9C11.9 7.9 11.8 7.9 11.6 7.9L10.1 6.4C10.2 6.4 10.2 6.3 10.3 6.3C11.5 5.9 12.7 5.9 13.6 6.1C14.1 6.2 14.5 6.4 14.9 6.6C15.6 7.3 16 8.1 16 9.3C16 10.1 15.7 10.8 15.2 11.4L13.5 9.8ZM11.8 15.9C12.5 15.9 13 16.2 13.3 16.8C13.4 17 13.4 17.2 13.4 17.4C13.4 17.8 13.2 18.2 13 18.5C12.7 18.8 12.3 18.9 11.9 18.9C11.5 18.9 11.1 18.7 10.8 18.5C10.5 18.2 10.4 17.8 10.4 17.4C10.4 17 10.6 16.6 10.8 16.3C11 16 11.4 15.9 11.8 15.9Z" /></svg>';
export const CHANGE_OWNERSHIP_FORM = {

      title: "APPLICATION FOR TRANSFER OF OWNERSHIP OF WATER LICENCE",
      description: "This form should be used to register the transfer of a licence for an existing abstraction to the new owner of the premises to which it applies.",
      logoPosition: "right",
      pages: [
        {
          name: "transferDetails",
          elements: [
            {
              type: "html",
              name: "applicationText",
              html: "<p>I/We* apply for change of ownership of licence to the new holder</p>"
            },
            {
              type: "text",
              name: "licenceReferenceNo",
              title: "Licence Reference No.",
              isRequired: true
            },
            {
              type: "text",
              name: "effectiveDate",
              title: "Effective Date of Transfer",
              inputType: "date",
              isRequired: true
            },
            {
              type: "signaturepad",
              name: "newHolderSignature",
              title: "Name and Signature of new holder",
              isRequired: true
            },
            {
              type: "text",
              name: "signatureDate",
              title: "Date",
              inputType: "date",
              isRequired: true
            }
          ]
        },
        {
          name: "newHolderDetails",
          elements: [
            {
              type: "panel",
              name: "holderInfoPanel",
              title: "1. Names and addresses of new holder (or new holder's agent)",
              elements: [
                {
                  type: "text",
                  name: "name",
                  title: "Name",
                  isRequired: true
                },
                {
                  type: "text",
                  name: "address",
                  title: "Address",
                  isRequired: true
                },
                {
                  type: "text",
                  name: "telephone",
                  title: "Telephone",
                  isRequired: true
                },
                {
                  type: "text",
                  name: "mobileNumber",
                  title: "Mobile Number"
                },
                {
                  type: "text",
                  name: "email",
                  title: "Email",
                  inputType: "email"
                },
                {
                  type: "text",
                  name: "designation",
                  title: "Designation"
                },
                {
                  type: "text",
                  name: "postalAddress",
                  title: "Postal address (if different from above)"
                },
                {
                  type: "text",
                  name: "district",
                  title: "District",
                  isRequired: true
                },
                {
                  type: "text",
                  name: "mainActivity",
                  title: "Main activity",
                  isRequired: true
                }
              ]
            }
          ]
        },
        {
          name: "transferReasons",
          elements: [
            {
              type: "comment",
              name: "transferReasons",
              title: "2. Reasons for transfer",
              description: "Please give your reasons for transferring this permit to other persons or company.",
              isRequired: true
            }
          ]
        },
        {
          name: "declaration",
          elements: [
            {
              type: "file",
              name: "attachments",
              title: "Attachments",
              description: "Please attach any supporting documents.",
              storeDataAsText: false,
              allowMultiple: true,
              maxSize: 102400
            },
            {
              type: "checkbox",
              name: "declaration",
              title: "Declaration of the Applicant",
              isRequired: true,
              choices: [
                {
                  value: "declare",
                  text: "I/We* certify that the information provided in this form is complete and correct to the best of my/our* knowledge."
                }
              ]
            },
            {
              type: "signaturepad",
              name: "applicantSignature",
              title: "Signature of Applicant",
              isRequired: true
            },
            {
              type: "text",
              name: "applicantName",
              title: "Names in full",
              isRequired: true
            },
            {
              type: "text",
              name: "declarationDate",
              title: "Date",
              inputType: "date",
              isRequired: true
            },
            {
              type: "html",
              name: "submissionInfo",
              html: "<div style='margin-top: 20px;'><p>When you have completed this form you must return and attach a copy of bank cash deposit slip or cheque for the processing fee and send it to:</p><p>The Executive Director,<br>National Water Resources Authority,<br>Private Bag 363,<br>Lilongwe 3,<br>Telephone: +265 (0) 111 647 907,<br>Email: nwrasec@gmail.com</p></div>"
            }
          ]
        }
      ],
      showQuestionNumbers: "off",
      questionErrorLocation: "bottom",
      completeText: "Submit Application",
      widthMode: "responsive",
      showNavigationButtons: "both",
      completedHtml: "<h3>Thank you for submitting your transfer application</h3><p>The Authority may contact you for further information regarding your application.</p>"
    };

    /*const survey = new Survey.Model(surveyJson);

    // Add custom validation for effective date (must be within last 21 days)
    survey.onValidateQuestion.add((sender, options) => {
      if (options.name === "effectiveDate") {
        const transferDate = new Date(options.value);
        const today = new Date();
        const diffTime = Math.abs(today.getTime() - transferDate.getTime());
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

        if (diffDays > 21) {
          options.error = "Transfer must be reported within 21 days of the effective date.";
        }
      }
    }); */

